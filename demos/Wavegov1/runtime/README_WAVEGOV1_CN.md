...
[WAVE-Go 仓库](https://github.com/vigorlee/wave-go/tree/Wavegov1) · [版本下载](https://github.com/vigorlee/wave-go/releases/tag/Wavegov1-baseline) · [完整验收](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/continuous_acceptance.json) · [依赖版本](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/snapshot.json)

</div>

![Wavegov1 四窗口：MuJoCo、MID360 点云、真实模型感知和 UE 场景](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/four_windows_continuous.png)
...
[验收汇总](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/continuous_acceptance.json) · [第 1 轮结果与轨迹](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/continuous_run_1/) · [第 2 轮结果与轨迹](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/continuous_run_2/)。高度来自实际 `/odom/mujoco_odom`，通过判定同时检查阶段、任务返回与上下坡实际高度变化。本表只统计最终配置，两轮结果不代表长期成功率。

本次修复了三处衔接：楼梯小幅偏航时过早降速；楼顶回追身后的临时通过点；坡脚路径贴近原有桶。保留原地形，把坡脚绕行提前到下坡末段。调参期间存在失败运行，最终证据与旧版本历史结果分开保存。

![实际连续上楼，同时显示 MID360 和真实模型输出](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/four_windows_stairs_up.png)

实测原始与语义 MID360 在 12 秒内各收到 **120 个不同时间戳的帧**，约 **10.03 / 10.02 Hz**，单帧中位数 **20,001 点**，最长间隔 **0.138 / 0.151 s**。见 [本轮点云刷新记录](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/continuous_refresh_audit.json)。[61 项回归测试通过](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/tests.log)。另保留 [早期单独上楼记录](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/climb_result.json) 和 [模型采样统计](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/perception_summary.json) 供对照。

![连续下坡，已提前绕开坡脚原有的桶](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/four_windows_ramp_down.png)
...
实际加载 Depth Anything V2 Small、Grounding DINO Tiny、SAM 2.1 Hiera Tiny。权重 revision 和 SHA-256 固定在 [manifest](models/anything/manifest.json)。Depth Anything 输出是相对逆深度，不能当作米制测距；SAM 掩膜类别来自 DINO，未用场景真值改写模型预测。

MID360 回调独立于模型推理，RViz `Decay Time: 0` 保留最新帧。[实际点云样本](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/mid360/) 提供下楼、平地和坡道的 NPZ 点坐标及标签。语义只在时间差 ≤ 0.6 秒且空间距离 ≤ 0.12 米时关联；过期或未匹配点为灰色 unknown。关闭的是 UE 调试扫描点，RViz 点云与语义分类仍正常发布。

![上楼过程中的原始模型输出，保留误检和漏检](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/evidence/stairs_03_models.png)
...
本目录保存的是可校验的源代码、配置及场景覆盖层。UE/MuJoCo 运行包、机器人网格、ROS 工作区安装产物与模型权重需在工作站已有；不包含这些大体积依赖。精确上游提交见 [snapshot.json](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/snapshot.json)，Python 核心依赖实测版本见 [perception-versions.txt](https://github.com/vigorlee/wave-go/blob/Wavegov1/demos/Wavegov1/perception-versions.txt)。
