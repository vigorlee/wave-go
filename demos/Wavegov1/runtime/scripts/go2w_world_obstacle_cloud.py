...
DEFAULT_SCENE = ROOT / "matrix/src/robot_mujoco/zsibot_robots/go2w/scene_terrain_yard.xml"
DEFAULT_JSON = ROOT / "matrix/scene/scene.json"
...
def parse_static_cylinders(scene: Path, allow_empty: bool = False) -> tuple[StaticCylinder, ...]:
    ...
    if not cylinders and not allow_empty:
...
class WorldObstacleCloud(Node):
    def __init__(self) -> None:
        ...
        if requested_json:
            ...
            try:
                ...
                raise
        self.native_people = os.environ.get("GO2W_NATIVE_PEOPLE") == "1"
        self.static_cylinders = parse_static_cylinders(self.scene, allow_empty=self.native_people)
        self.state_relay = None
        if os.environ.get("GO2W_MJ_STATE_RELAY") == "1":
            from go2w_mujoco_state_relay import MujocoStateRelay
            self.state_relay = MujocoStateRelay(self.scene)
        self.native_people = os.environ.get("GO2W_NATIVE_PEOPLE") == "1"
        self.crossing_yield = None
        if self.state_relay and not self.native_people:
            from go2w_dynamic_yield import CrossingYield
            self.crossing_yield = CrossingYield(self.scene)
        self.yield_file = ROOT / ".run/go2w/dynamic_yield"
        self.yield_status = {"active": False, "actors": []}
    ...
    def actor_positions(self, elapsed_s: float) -> tuple[tuple[str, tuple[float, float, float]], ...]:
        if self.state_relay is not None:
            positions = self.state_relay.positions()
            return () if self.native_people else positions
        return () if self.native_people else tuple((actor.name, actor.at(elapsed_s)) for actor in self.actors)
    ...
    def publish_status(self, positions: tuple[tuple[str, tuple[float, float, float]], ...], modeled: int) -> None:
        now = time.monotonic()
        if now - self.last_status_at < (0.1 if self.state_relay else 1.0):
            return
        self.last_status_at = now
        actual_robot = self.state_relay.latest[2][:2] if self.state_relay and self.state_relay.latest else None
        payload = {
            "scene": str(self.scene),
            "scene_json": str(self.scene_json) if self.scene_json else None,
            "dynamic_actor_count": len(positions),
            "crossing_yield": self.yield_status,
            "dynamic_position_source": "native_humans_sensor_only" if self.native_people else ("mujoco_qpos_relay" if self.state_relay else "json_time_model"),
            "native_people": self.native_people,
            "sim_time": self.state_relay.latest[1] if self.state_relay and self.state_relay.latest else None,
            "dynamic_actors": [
                {"name": name, "x": round(float(position[0]), 3), "y": round(float(position[1]), 3),
                 "robot_center_distance_m": math.dist(actual_robot, position[:2]) if actual_robot else None}
    ...
    def on_cloud(self, message: PointCloud2) -> None:
        ...
        try:
            ...
            return
        if self.crossing_yield and self.state_relay.latest:
            _, sim_time, qpos = self.state_relay.latest
            active, actors = self.crossing_yield.evaluate(sim_time, qpos, positions)
            self.yield_status = {"active": active, "actors": actors}
            self.yield_file.parent.mkdir(parents=True, exist_ok=True)
            temporary = self.yield_file.with_suffix(".tmp")
            temporary.write_text(f"{time.time()+1.0:.6f} {int(active)}\n")
            temporary.replace(self.yield_file)
...
def main() -> None:
    ...
    try:
        ...
        if node.state_relay is not None:
            node.state_relay.close()
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()
